...
Waypoint can register listeners as [Tailscale Services](https://tailscale.com/kb/1438/tailscale-services) using the `service` field. <TestRef name="TestLoad_ServiceField" /> This gives each listener its own stable FQDN (e.g. `waypoint-db.tailf3d5b.ts.net`) that is load-balanced across all nodes hosting the service. <TestRef name="TestIntegration_ListenService_MultiInstance" />
...
Waypoint validates this at config load time and rejects the configuration with an error. <TestRef name="TestValidate_ServiceHostnameCollision" /> Distinct hostnames pass the check. <TestRef name="TestValidate_ServiceHostnameNoCollision" /> To fix it, use a different hostname:
...
- Service hosts **must be tagged nodes**. Untagged nodes receive `ErrUntaggedServiceHost`. <TestRef name="TestIntegration_ListenService_UntaggedNodeFails" /> Tagged nodes register successfully. <TestRef name="TestIntegration_ListenService_TaggedNode" />
