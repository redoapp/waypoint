...
Kubernetes mode follows the official Tailscale API proxy more closely:
`WhoIs` and the listener grant are evaluated on every HTTP request rather than
on this interval. A streaming exec/watch request keeps the identity established
when that request began; the next request sees policy changes immediately.
