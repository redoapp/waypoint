...
func TestNewProvisioner_DefaultPrefix(t *testing.T) {
	p := NewProvisioner("admin", "pass", "postgres", "localhost:5432", "", false, nil, nil, nil, nil)
	if p.userPrefix != "wp_" {
...
func TestNewProvisioner_CustomPrefix(t *testing.T) {
	p := NewProvisioner("admin", "pass", "postgres", "localhost:5432", "custom_", false, nil, nil, nil, nil)
